...
function code = arithmetic_encoding(seq, num_condition,upperbound,never_occur,minimum) % k-ary, X is encoded by C(k,b)    
    ...
    for i = 1 : length(seq)
        ...
        S = local_frequency_table(i, seq, num_condition, prob);
    end
    b = 2;
    while(1)
...
function seq = arithmetic_decoding(N, code, num_condition,upperbound,never_occur,minimum)
    ...
    while(i < N)
        ...
        while(in_range)
            in_range = false;
            array = lower + (upper - lower) * S(condition,:);
...
function S = local_frequency_table(i, seq, num_condition, local_prob)
    switch seq(1,i) % 31
    ...
    S=[zeros(num_condition,1),cumsum(local_prob')'];
end